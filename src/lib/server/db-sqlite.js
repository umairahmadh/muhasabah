...
export async function listProjects(status = 'active') {
	...
	const doneToday = db.prepare(`SELECT COUNT(*) n FROM tasks WHERE project_id = ? AND done = 1 AND date(done_at) = date('now','localtime')`);
	const hot       = db.prepare(`SELECT COUNT(*) n FROM tasks WHERE project_id = ? AND done = 0 AND starred = 1`);
	const nextTasks = db.prepare(`SELECT title FROM tasks WHERE project_id = ? AND done = 0 ORDER BY starred DESC, position ASC, created_at ASC LIMIT 3`);
...
export async function createProject(name, note = '', color = '#6ea8fe') {
	return db.prepare(`INSERT INTO projects (name, note, color) VALUES (?, ?, ?)`).run(name, note, color).lastInsertRowid;
}
...
export async function listTasks(projectId) {
	return db
		.prepare(`SELECT * FROM tasks WHERE project_id = ? ORDER BY done ASC, starred DESC, position ASC, created_at ASC`)
		.all(projectId);
}
